...
/** Build the bracket PDF and return the document + filename WITHOUT saving. */
export function build_bracket_pdf(
...
/** Build the results PDF and return the document + filename WITHOUT saving. */
export function build_results_pdf(
  tournament: Tournament,
  matches: Match[],
  competitors: Competitor[],
  category_name?: string,
...
/** Build then immediately trigger the browser save dialog for the bracket PDF. */
export function export_bracket_pdf(
  ...
  const { doc, filename } = build_bracket_pdf(tournament, matches, competitors, category_name);
  doc.save(filename);
}

/** Build then immediately trigger the browser save dialog for the results PDF. */
export function export_results_pdf(
  ...
  const { doc, filename } = build_results_pdf(tournament, matches, competitors, category_name);
  doc.save(filename);
}
